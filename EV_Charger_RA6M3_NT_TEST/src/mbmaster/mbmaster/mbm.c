...
#define MBM_MAX_HANDLES                                                                          \
    ( ( ( ( MBM_ASCII_ENABLED ) ? /*lint -e(506) */ 1 : 0 ) * MBM_SERIAL_ASCII_MAX_INSTANCES ) + \
      ...
      ( ( ( MBM_TCP_ENABLED ) ? /*lint -e(506) */ 1 : 0 ) * MBM_TCP_MAX_INSTANCES ) + ( MBM_TEST_INSTANCES ) )
...
pxMBMGetNewHdl( void )
{
    ...
    for( ubIdx = 0; ubIdx < MB_UTILS_NARRSIZE( xMBMInternalHdl ); ubIdx++ )
    {
        if( IDX_INVALID == xMBMInternalHdl[ubIdx].ubIdx )
        {
            ...
            if( MB_ENOERR != ( eStatus2 = eMBPTimerInit( &( pxIntHdl->xRespTimeoutHdl ), MBM_TIMEOUT_RESOLUTION_MS,
                                                         bMBMResponseTimeoutCB, pxIntHdl ) ) )
#else
            if( MB_ENOERR != ( eStatus2 = eMBPTimerInit( &( pxIntHdl->xRespTimeoutHdl ), MBM_DEFAULT_RESPONSE_TIMEOUT,
                                                         bMBMResponseTimeoutCB, pxIntHdl ) ) )
#endif
            {
                eStatus = eStatus2;
...
void
vMBMMasterTransactionPolled( xMBMInternalHandle *pxIntHdl, UCHAR ucSlaveAddress, eMBMQueryState *peState,
                             eMBErrorCode *peStatus )
{
    xMBPEventType xFrameEvent;

#if defined( MBM_ENABLE_DEBUG_FACILITY ) && ( MBM_ENABLE_DEBUG_FACILITY == 1 )
    eMBMQueryState eOldState = *peState;
#endif
    ...
    switch( *peState )
    {
        case MBM_STATE_SEND:
            ...
            if( 0 )
            {
                     ...
                     ( *peStatus = pxIntHdl->pFrameSendFN( pxIntHdl, ucSlaveAddress, pxIntHdl->usFrameMBPDULength ) ) )
            {
                *peState = MBM_STATE_ERROR;
            }
            else
            ...
            if( bMBPEventGet( pxIntHdl->xFrameEventHdl, &xFrameEvent ) )
            {
                ...
                switch( ( eMBMEvent )xFrameEvent )
                {
                    case MBM_EV_TIMEDOUT:
                        ...
                        if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
                        {
                            vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                         "[IDX=" MBP_FORMAT_USHORT "] Request timeout. Reseting state.\n",
                    ...
                    case MBM_EV_RECEIVED:
                        ...
                        {
                            vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                         "[IDX=" MBP_FORMAT_USHORT "] Response frame received.\n",
                                         ( USHORT )pxIntHdl->ubIdx );
                        }
#endif
                        if( MB_ENOERR != ( *peStatus = pxIntHdl->pFrameRecvFN( pxIntHdl, ucSlaveAddress,
                    ...
                    default:
                        ...
                        if( bMBPPortLogIsEnabled( MB_LOG_ERROR, MB_LOG_CORE ) )
                        {
                            vMBPPortLog( MB_LOG_ERROR, MB_LOG_CORE,
                                         "[IDX=" MBP_FORMAT_USHORT "] Illegal event received!\n",
                                         ( USHORT )pxIntHdl->ubIdx );
                        }
#endif
                        *peState = MBM_STATE_ERROR;
...
#if defined( MBM_ENABLE_DEBUG_FACILITY ) && ( MBM_ENABLE_DEBUG_FACILITY == 1 )
           if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) && (*peState != eOldState ) )
           {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE, "Transaction polled %d -> %d\n", eOldState, *peState );
           }
#endif
...
eMBErrorCode
eMBMSerialInitExt( xMBMHandle *pxHdl, eMBSerialMode eMode, UCHAR ucPort, ULONG ulBaudRate, eMBSerialParity eParity,
                   UCHAR ucStopBits )
{
