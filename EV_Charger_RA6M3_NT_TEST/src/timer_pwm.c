...
fsp_err_t gpt_initialize(void)
{
    ...
    /* Open GPT module */
    err = R_GPT_Open (&g_timer_485_ctrl, &g_timer_485_cfg);
    if (FSP_SUCCESS != err)
    {
...
fsp_err_t gpt_start(void)
{
    ...
    /* Start GPT module */
    err=  R_GPT_Start (&g_timer_485_ctrl);
    if (FSP_SUCCESS != err)
    {
...
fsp_err_t set_intensity(uint32_t raw_count, uint8_t pin)
{
    ...
    err = R_GPT_DutyCycleSet (&g_timer_485_ctrl, raw_count, pin);
    if (FSP_SUCCESS != err)
    {
...
void timer_gpt_deinit(void)
{
    ...
    err = R_GPT_Close (&g_timer_485_ctrl);
    if (FSP_SUCCESS != err)
    {
