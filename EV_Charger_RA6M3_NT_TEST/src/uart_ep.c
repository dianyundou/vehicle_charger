...
#define UART_TE_ENABLE                          (R_IOPORT_PinWrite(&g_ioport_ctrl, BSP_IO_PORT_11_PIN_01, BSP_IO_LEVEL_HIGH))
#define UART_TE_DISABLE                         (R_IOPORT_PinWrite(&g_ioport_ctrl, BSP_IO_PORT_11_PIN_01, BSP_IO_LEVEL_HIGH))
#define UART_RE_ENABLE                          (R_IOPORT_PinWrite(&g_ioport_ctrl, BSP_IO_PORT_11_PIN_01, BSP_IO_LEVEL_HIGH))
#define UART_RE_DISABLE                         (R_IOPORT_PinWrite(&g_ioport_ctrl, BSP_IO_PORT_11_PIN_01, BSP_IO_LEVEL_HIGH))
/*
...
fsp_err_t uart_initialize(void)
{
    ...
    err = R_SCI_UART_Open (&g_uart_485_ctrl, &uart_cfg);
    if (FSP_SUCCESS != err)
    {
...
void uart_config_set(eMBSerialParity eParity)
{

    memcpy(&uart_cfg, &g_uart_485_cfg, sizeof(uart_cfg_t));
 ...
 * @retval      Any Other Error code apart from FSP_SUCCESS  Unsuccessful open
 ***********************************************************************************************************************/
fsp_err_t uart_te_disable(void)
{
    fsp_err_t err = FSP_SUCCESS;

    sci_uart_instance_ctrl_t * p_ctrl = (sci_uart_instance_ctrl_t *) &g_uart_485_ctrl;

    /* */
    UART_TE_DISABLE;

    p_ctrl->p_reg->SCR &= (uint8_t) ~(SCI_SCR_TIE_MASK);
 ...
 * @retval      Any Other Error code apart from FSP_SUCCESS  Unsuccessful open
 ***********************************************************************************************************************/
fsp_err_t uart_re_disable(void)
{
    fsp_err_t err = FSP_SUCCESS;
    ...
    /* */
    UART_RE_DISABLE;
    p_ctrl->p_reg->SCR &= (uint8_t) ~(SCI_SCR_RIE_MASK);
...
fsp_err_t uart_re_enable(void)
{
    ...
    /* */
    UART_RE_ENABLE;
    p_ctrl->p_reg->SCR |= (SCI_SCR_RIE_MASK);
...
fsp_err_t uart_print_user_msg(uint8_t *p_msg)
{
    ...
    /* Writing to terminal */
    err = R_SCI_UART_Write (&g_uart_485_ctrl, p_msg, msg_len);
    if (FSP_SUCCESS != err)
    {
...
void deinit_uart(void)
{
    ...
    /* Close module */
    err =  R_SCI_UART_Close (&g_uart_485_ctrl);
    if (FSP_SUCCESS != err)
    {
